SP500_FALLBACK = [
    'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'META', 'TSLA', 'BRK.B',
    'UNH', 'JNJ', 'XOM', 'V', 'JPM', 'WMT', 'PG', 'MA', 'HD', 'CVX',
    'LLY', 'ABBV', 'MRK', 'AVGO', 'KO', 'PEP', 'COST', 'ADBE', 'MCD',
    'CSCO', 'ACN', 'TMO', 'ABT', 'NFLX', 'DHR', 'VZ', 'NKE', 'WFC',
    'DIS', 'CMCSA', 'TXN', 'PM', 'CRM', 'BMY', 'NEE', 'RTX', 'ORCL',
    'INTC', 'UPS', 'HON', 'QCOM', 'BA', 'LOW', 'SPGI', 'BLK', 'LMT',
    'AMD', 'INTU', 'CAT', 'DE', 'GE', 'AXP', 'ISRG', 'GILD', 'NOW',
    'BKNG', 'ADI', 'PLD', 'MDLZ', 'SYK', 'ADP', 'REGN', 'TJX', 'VRTX',
    'CB', 'SBUX', 'CI', 'TMUS', 'PYPL', 'MMC', 'SO', 'ZTS', 'SCHW',
    'MO', 'BSX', 'DUK', 'AMT', 'PGR', 'LRCX', 'EOG', 'ITW', 'BDX',
    'C', 'SLB', 'NOC', 'CME', 'MMM', 'USB', 'HUM', 'PNC', 'FI', 'TGT',
    'AON', 'MS', 'MCK', 'EQIX', 'APD', 'ELV', 'PSX', 'FCX', 'KLAC', 'SHW',
    'FDX', 'ICE', 'NSC', 'WM', 'EMR', 'GD', 'TFC', 'ROP', 'MAR', 'AEP',
    'CMI', 'HCA', 'OXY', 'VLO', 'KMI', 'MPC', 'PCAR', 'TT', 'CTAS', 'ECL',
    'COF', 'TRV', 'MET', 'AIG', 'ALL', 'BK', 'AMP', 'MSI', 'PAYX', 'AFL',
    'SYY', 'ROST', 'CPRT', 'DLTR', 'FIS', 'FISV', 'OTIS', 'CARR', 'ROK', 'JCI',
    'WELL', 'SPG', 'PSA', 'O', 'D', 'EXR', 'PEG', 'ED', 'XEL', 'WEC',
    'ANET', 'PANW', 'CRWD', 'SNPS', 'CDNS', 'WMB', 'MU', 'AMAT', 'LULU', 'REG',
    'A', 'IDXX', 'BIIB', 'ILMN', 'MRNA', 'CSX', 'UNP', 'ADSK', 'FAST', 'EA',
    'MNST', 'KHC', 'GIS', 'KMB', 'CL', 'CHD', 'HSY', 'MGM', 'RCL', 'CCL',
    'HLT', 'LVS', 'NUE', 'STLD', 'DOW', 'NEM', 'DD', 'PPG', 'LYB', 'PH',
    'SWK', 'ETN', 'IT', 'LEN', 'DHI', 'PHM', 'VMC', 'MLM', 'HPE', 'CVS',
    'WBA', 'KR', 'DGX', 'LH', 'RMD', 'DXCM', 'EW', 'SYF', 'DFS', 'RF',
    'HBAN', 'FITB', 'KEY', 'STT', 'NTRS', 'MTB', 'CFG', 'CNC', 'MOH', 'HIG',
    'GS', 'BAC', 'PFE', 'AMGN', 'MDT', 'COP', 'DVN', 'HAL', 'BKR', 'MRO',
    'APA', 'FANG', 'CTRA', 'OVV', 'GWW', 'IR', 'FTV', 'XYL', 'IEX', 'AME',
    'GNRC', 'TDY', 'HWM', 'BALL', 'IP', 'PKG', 'SEE', 'AVY', 'CF', 'MOS',
    'FMC', 'AWK', 'ES', 'ETR', 'FE', 'NI', 'AES', 'CMS', 'LNT', 'EVRG',
    'ARE', 'BXP', 'VTR', 'KIM', 'UDR', 'CPT', 'EQR', 'ESS', 'MAA', 'IRM',
    'INVH', 'AMH', 'CUBE', 'REXR', 'COLD', 'WPC', 'NNN', 'STOR', 'ADC',
    'ABC', 'CAH', 'BAX', 'IQV', 'CRL', 'CTLT', 'HOLX', 'TECH', 'VTRS',
    'JAZZ', 'NBIX', 'ALNY', 'BMRN', 'IONS', 'SGEN', 'RARE',
    'UBER', 'ABNB', 'DASH', 'RBLX', 'U', 'SNOW', 'DDOG', 'NET', 'OKTA',
    'ZS', 'CFLT', 'MDB', 'GTLB', 'HUBS', 'BILL', 'DOCU',
    'DG', 'FIVE', 'BURL', 'BBY', 'TPR', 'PVH', 'HBI', 'UAA', 'VFC',
    'GPS', 'ANF', 'AEO', 'URBN', 'RL', 'PVH',
    'NDAQ', 'CBOE', 'IVZ', 'BEN', 'TROW', 'RJF', 'PFG', 'PRU', 'GL',
    'LNC', 'CINF', 'EG', 'WRB', 'ACGL', 'RNR', 'RE',
]

IBEX_35 = [
    'ACS.MC', 'ACX.MC', 'AMS.MC', 'ANA.MC', 'ANE.MC',
    'BBVA.MC', 'BKT.MC', 'CABK.MC', 'CLNX.MC', 'COL.MC',
    'AENA.MC', 'ELE.MC', 'ENG.MC', 'FDR.MC', 'FER.MC',
    'GRF.MC', 'IAG.MC', 'IBE.MC', 'IDR.MC', 'ITX.MC',
    'LOG.MC', 'MAP.MC', 'MRL.MC', 'MTS.MC', 'NTGY.MC',
    'PUIG.MC', 'RED.MC', 'REP.MC', 'ROVI.MC', 'SAB.MC',
    'SAN.MC', 'SCYR.MC', 'SLR.MC', 'TEF.MC', 'UNI.MC',
]

EUROSTOXX_50 = [
    'ADYEN.AS', 'AD.AS', 'AIR.PA', 'AI.PA', 'ASML.AS',
    'ABI.BR', 'ASRNL.AS', 'BAS.DE', 'BAYN.DE', 'BBVA.MC',
    'BMW.DE', 'BNP.PA', 'CS.PA', 'DHL.DE', 'ENEL.MI',
    'ENI.MI', 'FLTR.IR', 'IBE.MC', 'IFX.DE', 'INGA.AS',
    'ISP.MI', 'ITX.MC', 'KER.PA', 'DG.PA', 'OR.PA',
    'MC.PA', 'MUV2.DE', 'NOKIA.HE', 'NDA-FI.HE', 'RI.PA',
    'PHIA.AS', 'PRX.AS', 'SAF.PA', 'SAN.PA', 'SAN.MC',
    'SAP.DE', 'SIE.DE', 'STLAM.MI', 'SU.PA', 'TTE.PA',
    'UCG.MI', 'VOW3.DE', 'VNA.DE', 'BN.PA', 'RMS.PA',
    'ALV.DE', 'DB1.DE', 'CRH.IR', 'DTE.DE', 'ENR.DE',
]

NIKKEI_225 = [
    '4151.T', '4502.T', '4503.T', '4506.T', '4507.T', '4519.T', '4523.T', '4568.T', '4578.T',
    '4062.T', '6479.T', '6501.T', '6503.T', '6504.T', '6506.T', '6526.T', '6645.T', '6674.T',
    '6701.T', '6702.T', '6723.T', '6724.T', '6752.T', '6753.T', '6758.T', '6762.T', '6770.T',
    '6841.T', '6857.T', '6861.T', '6902.T', '6920.T', '6952.T', '6954.T', '6963.T', '6971.T',
    '6976.T', '6981.T', '7735.T', '7751.T', '7752.T', '8035.T',
    '7201.T', '7202.T', '7203.T', '7205.T', '7211.T', '7261.T', '7267.T', '7269.T', '7270.T', '7272.T',
    '4543.T', '4902.T', '6146.T', '7731.T', '7733.T', '7741.T',
    '9432.T', '9433.T', '9434.T', '9984.T',
    '5831.T', '7186.T', '8304.T', '8306.T', '8308.T', '8309.T', '8316.T', '8331.T', '8354.T', '8411.T',
    '8253.T', '8591.T', '8697.T',
    '8601.T', '8604.T',
    '8630.T', '8725.T', '8750.T', '8766.T', '8795.T',
    '1332.T',
    '2002.T', '2269.T', '2282.T', '2501.T', '2502.T', '2503.T', '2801.T', '2802.T', '2871.T', '2914.T',
    '3086.T', '3092.T', '3099.T', '3382.T', '7453.T', '8233.T', '8252.T', '8267.T', '9843.T', '9983.T',
    '2413.T', '2432.T', '3659.T', '3697.T', '4307.T', '4324.T', '4385.T', '4661.T', '4689.T', '4704.T',
    '4751.T', '4755.T', '6098.T', '6178.T', '6532.T', '7974.T', '9602.T', '9735.T', '9766.T',
    '1605.T',
    '3401.T', '3402.T',
    '3861.T',
    '3405.T', '3407.T', '4004.T', '4005.T', '4021.T', '4042.T', '4043.T', '4061.T', '4063.T', '4183.T',
    '4188.T', '4208.T', '4452.T', '4901.T', '4911.T', '6988.T',
    '5019.T', '5020.T',
    '5101.T', '5108.T',
    '5201.T', '5214.T', '5233.T', '5301.T', '5332.T', '5333.T',
    '5401.T', '5406.T', '5411.T',
    '3436.T', '5706.T', '5711.T', '5713.T', '5714.T', '5801.T', '5802.T', '5803.T',
    '2768.T', '8001.T', '8002.T', '8015.T', '8031.T', '8053.T', '8058.T',
    '1721.T', '1801.T', '1802.T', '1803.T', '1808.T', '1812.T', '1925.T', '1928.T', '1963.T',
    '5631.T', '6103.T', '6113.T', '6273.T', '6301.T', '6302.T', '6305.T', '6326.T', '6361.T', '6367.T',
    '6471.T', '6472.T', '6473.T', '7004.T', '7011.T', '7013.T',
    '7012.T',
    '7832.T', '7911.T', '7912.T', '7951.T',
    '3289.T', '8801.T', '8802.T', '8804.T', '8830.T',
    '9001.T', '9005.T', '9007.T', '9008.T', '9009.T', '9020.T', '9021.T', '9022.T',
    '9064.T', '9147.T',
    '9101.T', '9104.T', '9107.T',
    '9201.T', '9202.T',
    '9501.T', '9502.T', '9503.T',
    '9531.T', '9532.T',
]
